...
% Use spm_reslice to reslice the combined ROI mask to the sample contrast image space
    ...
    for j = 1:num_conditions
        ...
        if isfile(con_file)
            ...
            if all(size(Y) == size(resliced_combined_roi_mask))
                ...
                beta_values = Y(resliced_combined_roi_mask > 0);
                if ~all(isnan(beta_values))
                    NF_betas(i, j) = mean(beta_values, 'omitnan'); % Mean beta value within the resliced combined ROI
                else
    ...
    for j = 1:num_conditions
        ...
        if isfile(con_file)
            ...
            if all(size(Y) == size(resliced_combined_roi_mask))
                ...
                beta_values = Y(resliced_combined_roi_mask > 0);
                if ~all(isnan(beta_values))
                    control_betas(i, j) = mean(beta_values, 'omitnan'); % Mean beta value within the resliced combined ROI
                else
