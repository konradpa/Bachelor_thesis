...
sham_contrast_values_007 = zeros(length(sham_subjects), 1);

% Extract contrast values for the sham group
for i = 1:length(sham_subjects)
    con_file_upregulation = fullfile(main_project_dir, sham_subjects{i}, 'Count_Upreg_fDisp', 'con_0007.nii');  % Upregulation > Counting contrast file
    ...
    if isfile(con_file_upregulation)
        ...
        if all(size(Y_upregulation) == size(resliced_combined_roi_mask))
            ...
            sham_contrast_values_007(i) = mean(contrast_values_upregulation, 'omitnan');
            
            % Print the extracted contrast values for debugging
            fprintf('Sham subject %s: Upregulation > Counting contrast value = %f\n', sham_subjects{i}, sham_contrast_values_007(i));
        else
...
[h, p, ci, stats] = ttest(sham_contrast_values_007);
...
figure;
scatter(ones(length(sham_contrast_values_007), 1), sham_contrast_values_007, 'filled', 'r'); % Scatter points
hold on;
mean_value = mean(sham_contrast_values_007);
...
% Calculate Cohen's d
std_dev = std(sham_contrast_values_007);
cohen_d = mean_value / std_dev;
fprintf('Cohen''s d: %.3f\n', cohen_d);

% Number of subjects
n = length(sham_contrast_values_007);
